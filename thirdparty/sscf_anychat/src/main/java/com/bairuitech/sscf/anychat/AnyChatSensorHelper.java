package com.bairuitech.sscf.anychat;
...
public class AnyChatSensorHelper implements SensorEventListener{
	...
	public void InitSensor(Context context) {
		...
		AnyChatCoreSDK.SetSDKOptionInt(com.bairuitech.sscf.anychat.AnyChatDefine.BRAC_SO_LOCALVIDEO_DEVICEMODE, iDeviceType);
		// 设置屏幕旋转方向
		...
		AnyChatCoreSDK.SetSDKOptionInt(com.bairuitech.sscf.anychat.AnyChatDefine.BRAC_SO_LOCALVIDEO_SURFACEROTATION, degrees);
	...
	public void onSensorChanged(SensorEvent event) {
		...
		if ((Math.abs(X - LastXSpead) <= 0.5) && (Math.abs(Y - LastYSpead) <= 0.5) && (Math.abs(Z - LastZSpead) <= 0.5)) // 静止状态
		{
			...
			if (bCameraNeedFocus && interval > 1000) {
				...
				if(AnyChatCoreSDK.GetSDKOptionInt(com.bairuitech.sscf.anychat.AnyChatDefine.BRAC_SO_LOCALVIDEO_CAPDRIVER) == com.bairuitech.sscf.anychat.AnyChatDefine.VIDEOCAP_DRIVER_JAVA)
					AnyChatCoreSDK.mCameraHelper.CameraAutoFocus();
				else
					AnyChatCoreSDK.SetSDKOptionInt(com.bairuitech.sscf.anychat.AnyChatDefine.BRAC_SO_LOCALVIDEO_FOCUSCTRL, 1);
			}
		} else {
...
//		int ANYCHAT_DEVICEORIENTATION_FACEDOWN			= 2;		// Device oriented flat, face down
		...
		AnyChatCoreSDK.SetSDKOptionInt(com.bairuitech.sscf.anychat.AnyChatDefine.BRAC_SO_LOCALVIDEO_ORIENTATION, orientation);
	}
}